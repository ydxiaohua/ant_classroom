...
public class UserController {
    ...
    public List<Menu> queryquanxian(){
        List<Menu> menu = userService.queryquanxian();
        return menu;
    }


    //删除权限
    @RequestMapping("/deletequanxian")
    @ResponseBody
    public String deletequanxian(Integer id){
        String asd = userService.deletequanxian(id);
        return asd;
    }
    ...
    public void addquanxian(Menu menu){
        userService.addquanxian(menu);
    }

    //回显信息
    @RequestMapping(value="/selectquanxianid")
    public String selectquanxianid(Integer id,HttpServletRequest request){
        Menu qx = userService.selectquanxianid(id);
        request.getSession().setAttribute("qx",qx);
        return "twl/updatequanxian.jsp";
    }
    ...
    @ResponseBody
    public String updateqx(Menu menu){
        userService.updateqx(menu);
        return "修改成功";
    }

    //验证加修改密码
    @RequestMapping(value="/yzmm")
    @ResponseBody
    public String yzmm(User user){
        String aa = userService.yzmm(user);
        return aa;
    }
