...
<body>
        ...
        <table class="table" id="wen" border="1"></table>
...
<script type="text/javascript">
    ...
    $("#wen").bootstrapTable({
        url:"<%=request.getContextPath()%>/kcmuser/queryWenList",
        striped: true,//隔行变色
        ...
        columns:[
            ...
            {field:'questionid',title:'主键id',width: 100},
            {field:'peoplename',title:'问题用户id',width: 100},
            {field:'questiontypeid',title:'问题类型',width: 100},
            {field:'questioncontent',title:'问题内容 ',width: 100},
            {field:'questionname',title:'问题标题',width: 50},
            {field:'questiondate',title:'提问时间',width: 150},
            {field:'act',title:'操作',width:200,
                formatter: function(value,row,index){
                    return '<a href="javascript:shanchuwen('+row.questionid+')" class="btn btn-danger" role="button">删除</a><a href="javascript:xiugaiwen('+row.questionid+')" class="btn btn-warning" role="button">修改</a>';
                }
            },
        ]
    })
    ...
    function tianjia(){
        BootstrapDialog.show({
            title: '添加问题信息',
            keyboard:true,
            message: $('<div></div>').load('<%=request.getContextPath()%>/kcm/addWen.jsp'),
            buttons: [{
                label: '保存',
                action: function() {
                    $.ajax({
    ...
    function shanchuwen(id){
        BootstrapDialog.show({
            ...
            buttons: [{
                ...
                action: function() {
                    $.ajax({
                        url:"<%=request.getContextPath()%>/kcmuser/deletewen",
                        type:"post",
                        data:{"id":id},
    ...
    function xiugaiwen(wenid){
        BootstrapDialog.show({
            title: '修改问题信息',
            message: $('<div></div>').load('<%=request.getContextPath()%>/kcmuser/huixianwen?wenid='+wenid),
            buttons: [{
                label: '保存',
                action: function() {
                    $.ajax({
                        url:"<%=request.getContextPath()%>/kcmuser/updatewen",
                        type:"post",
