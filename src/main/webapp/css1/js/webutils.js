...
function initKindEditor_addblog(id, width, height, param, pressText) {
	EditorObject = KindEditor.create('textarea[id=' + id + ']', {
		...
		minWidth : '10px',
		minHeight : '10px',
		uploadJson : keuploadSimpleUrl + '&param=' + param
				+ '&fileType=jpg,gif,png,jpeg&pressText=' + pressText,// 图片上传路径
		afterBlur : function() {
			this.sync();
		},
		allowFileManager : false,
		items : [ 'source', 'fontname', 'fontsize', '|', 'forecolor',
				'hilitecolor', 'bold', 'italic', 'underline', 'formatblock',
 ...
 *            图片上传目录名
 * @param callback
 *            上传成功后的回调函数，函数接收一个参数（上传图片的URL）
 * @param pressText
 *            是否上水印 false或空 否 true是
 */
	...
	KindEditor.ready(function(K) {
		var uploadbutton = K.uploadbutton({
			...
			url : uploadSimpleUrl + '&param=' + param
					+ '&fileType=jpg,gif,png,jpeg&pressText=' + pressText,
